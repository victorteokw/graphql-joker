const argsToSchemaDefs = require('../argsToSchemaDefs');
...
module.exports = ({ args, options, projDir }) => {
  ...
  if (options.destroy) {
    rm(destination(`${options.modelDir}/${schemaDefs.modelName}.js`));
    ...
    writeToFile(
      generateMongooseSchema(schemaDefs, 'model'),
      destination(`${options.modelDir}/${schemaDefs.modelName}.js`)
    );
    writeToFile(
      generateGraphQLSchema(schemaDefs, 'model'),
      destination(`${options.schemaDir}/${schemaDefs.modelName}.gql`)
    );
    writeToFile(
      generateResolver(schemaDefs, 'model'),
      destination(`${options.resolverDir}/${schemaDefs.modelName}.js`)
    );
    writeToFile(
      generateModelTest(schemaDefs),
      destination(`tests/models/${schemaDefs.modelName}Test.js`)
    );
  }
};
