...
module.exports = ({ args, options, projDir }) => {
  ...
  if (Object.keys(options.nonDefaultOptions).length > 1) {
    updateJSON(
  ...
  orm.createFiles.forEach((f) => {
    if (f[0].call) {
      writeToFile(f[0](options), f[1].call ? destination(f[1])(options) : destination(f[1]), { soft: !options.overwrite });
    } else {
      copyTpl(f[0], destination(f[1]), options, { soft: !options.overwrite });
    }
  });
