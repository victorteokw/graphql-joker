...
const pushCommand = (command) => {
  ...
  if (isSingleCommand(command)) {
    ...
    separateArrayCommand(command).map((c) => userCommands.push(c));
  }
};

const pushCommands = (commands) => {
  commands.map((command) => pushCommand(command));
};

const executeAllCommands = (destDir) => {
  const executionCommands = convertCommandsToExecutionCommands(userCommands, destDir);
