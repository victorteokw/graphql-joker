...
const transformModifiers = (modifiers) => {
  ...
  if (modifiers.match) {
    ...
    modifiers.validate.is = modifiers.match;
  }
  if (modifiers.min) {
    if (!newModifiers.validate) newModifiers.validate = {};
    modifiers.validate.min = modifiers.min;
  }
  if (modifiers.max) {
    if (!newModifiers.validate) newModifiers.validate = {};
    modifiers.validate.max = modifiers.max;
  }
  if (modifiers.minlength && modifiers.maxlength) {
    if (!newModifiers.validate) newModifiers.validate = {};
    modifiers.validate.len = [modifiers.minlength, modifiers.maxlength];
  }
...
const schemaLine = (field, indentLevel = 1, indentSpace = 2) => {
  ...
  if (field.primitive) {
    if (Object.keys(field.modifiers).length === 0) {
      ...
      line += `{ type: Sequelize.${field.jsType}, ${transformModifiers(formatModifiers(field.modifiers))} }`;
    }
    ...
    if (Object.keys(field.modifiers).length === 0) {
      ...
      line += `{ type: Sequelize.INTEGER, model: '${field.jsType}', ${transformModifiers(formatModifiers(field.modifiers))} }`;
    }
  }
  return line;
};
