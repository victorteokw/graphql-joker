...
const resolverBody = (modelName, fields, orm, userPrimaryKey, indentSpace = 2) => {
  let final = '';
  ...
  fields.forEach((f, i) => {
    ...
    if (f.foreignKey) {
      ...
      if (f.isArray) {
        final += indentEveryLine(orm.resolverFunctions.getByIds(f.jsType, f.name, 'root', userPrimaryKey), 3, 2) + '\n';
      } else {
        final += indentEveryLine(orm.resolverFunctions.getById(f.jsType, f.name, 'root', userPrimaryKey), 3, 2) + '\n';
      }
    }
...
const resolverBodies = (modelName, fields, variant, orm, userPrimaryKey, indentSpace = 2) => {
  const roots = rootsThatRequiresBody(modelName, fields, indentSpace);
  let str = roots.map(({ modelName, fields }) =>
    resolverBody(modelName, fields, orm, userPrimaryKey, indentSpace)).join(',\n');
...
const generateResolver = (schemaDefs, variant, orm, userPrimaryKey) => {
  if (variant === 'schema' && !needsModelBody(schemaDefs.fields)) {
    return undefined;
  }
  return wrapper([
    needsModelBody(schemaDefs.fields) ? resolverBodies(schemaDefs.modelName, schemaDefs.fields, variant, orm, userPrimaryKey) : undefined,
    variant === 'model' ? queryBody(schemaDefs, orm, userPrimaryKey) : undefined,
    variant === 'model' ? mutationBody(schemaDefs, orm, userPrimaryKey) : undefined
  ]);
};
