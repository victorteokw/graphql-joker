...
const resolverBody = (orm, modelName, fields, indentSpace = 2) => {
  let final = '';
  final += `${' '.repeat(indentSpace)}${modelName}: {\n`;
  fields.forEach((f, i) => {
    if (orm.supportedPrimitiveTypes.includes(f.jsType)) return;
    if (f.assocModel) {
...
const resolverBodies = (orm, modelName, fields, variant, indentSpace = 2) => {
  const roots = rootsThatRequiresBody(modelName, fields, indentSpace);
  let str = roots.map(({ modelName, fields }) =>
    resolverBody(orm, modelName, fields, indentSpace)).join(',\n');
...
const generateResolver = (orm, schemaDefs, variant) => {
  if (variant === 'schema' && !needsModelBody(schemaDefs.fields)) {
    return undefined;
  }
  return wrapper([
    needsModelBody(schemaDefs.fields) ? resolverBodies(orm, schemaDefs.modelName, schemaDefs.fields, variant) : undefined,
