...
const requiringSchemaTypes = ({ objectId, mixed, file }) => {
  if (!objectId && !mixed && !file) return undefined;
  const types = [];
  if (objectId) types.push('ObjectId');
  if (mixed) types.push('Mixed');
  if (file) types.push('File');
...
const uploaderRequirement = (uploader) =>
  `const ${uploader} = require('../uploaders/${uploader}');`;

const uploaderRequirements = (uploaders) =>
  uploaders.map(uploaderRequirement).join('\n');
...
const requiresObjectId = (fields) => {
  ...
  for (const f of fields) {
    ...
    } else if (!f.primitive && !f.foreignKey && !f.isObject && !f.isSchema && !f.assocModel) {
      requires = true;
    }
...
const requiresFile = (fields) => {
  let requires = false;
  for (const f of fields) {
    if (f.fields) {
      requires = requiresFile(f.fields);
    } else if (f.primitive && f.jsType === 'File') {
      requires = true;
    }
    if (requires) return true;
  }
...
const requiredUploaders = (fields) => {
  const retval = [];
  const _requiredUploaders = (fields) => {
    fields.forEach((field) => {
      field.fields && _requiredUploaders(field.fields);
      field.jsType === 'File' && !retval.includes(field.modifiers.uploader) && retval.push(field.modifiers.uploader);
    });
  };
  _requiredUploaders(fields);
  return retval;
};

// variant: model, schema
const generateModel = (schemaDefs, variant) =>
  concatBlocks([
    topOfHeader,
    requiringSchemaTypes({
      objectId: requiresObjectId(schemaDefs.fields),
      mixed: requiresMixed(schemaDefs.fields),
      file: requiresFile(schemaDefs.fields)
    }),
    schemaRequirements(requiredSchemas(schemaDefs.fields)),
    uploaderRequirements(requiredUploaders(schemaDefs.fields)),
    '\n',
