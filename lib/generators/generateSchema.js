...
const extraSchemaTypes = (modelName, fields, primaryKey, indentSpace = 2) => {
...
const selfTypeDefinition = (schemaDefs, variant, primaryKey) => {
  const lines = [
...
const generateSchema = (schemaDefs, variant, options = {}) =>
  concatBlocks([
    needsExtraSchemaTypes(schemaDefs.fields) ? extraSchemaTypes(schemaDefs.modelName, schemaDefs.fields, options.primaryKey) + '\n' : undefined,
    selfTypeDefinition(schemaDefs, variant, options.primaryKey),
    variant === 'model' ? '\n' : undefined,
